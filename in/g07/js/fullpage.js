	...
	window.fullpage = {
		...
		setFullheight : function()  {
			_fullpage.find(".page.fullpage").each(function() {
				if($(this).outerWidth() <= 1400) {
					$(this).css({"height":'auto'});
				} else {
					var fullheight = $(window).outerHeight();
					$(this).css({"height":fullheight-_paddingTop});
				...
				if( _scrAble ) {
					...
					if( e.delta < 0 ) {
						if( _pageIdx == 1 || _pageIdx == 0 ) {
							...
							_pageIdx--;
							$("html,body").animate({"scrollTop":_fullpage.find("[data-pageIdx="+_pageIdx+"]").offset().top - _paddingTop}, _scrAnimSpeed, _scrAnimEasing, function() {
								_scrAble = true;
							});
						}
					} else {
